...
# ── Catalog Mistakes: folder or zip sources (same pattern as other KPI sources)
# Create a folder named exactly as the key, or a zip named "<key>.zip"
# e.g.  "Catalog Mistakes UAE All/"  OR  "Catalog Mistakes UAE All.zip"
MISTAKES_SOURCES = [
    "Catalog Mistakes UAE All",
    "Catalog Mistakes Jordan All",
    "Catalog Mistakes UAE Build",
    "Catalog Mistakes UAE Update",
    "Catalog Mistakes Jordan Build",
    "Catalog Mistakes Jordan Update",
]

# Display labels (shown as tab names)
MISTAKES_LABELS = {
    "Catalog Mistakes UAE All":       "UAE – All",
    "Catalog Mistakes Jordan All":    "Jordan – All",
    "Catalog Mistakes UAE Build":     "UAE – Build",
    "Catalog Mistakes UAE Update":    "UAE – Update",
    "Catalog Mistakes Jordan Build":  "Jordan – Build",
    "Catalog Mistakes Jordan Update": "Jordan – Update",
}
...
# ============================================================
# CATALOG MISTAKES LOADER  (folder / zip — same pattern as KPI sources)
# ============================================================
...
def _read_one_mistakes_file(path: Path) -> pd.DataFrame:
    if path.suffix.lower() in (".xlsx", ".xls"):
        return _normalize_mistakes_df(pd.read_excel(path))
    return _normalize_mistakes_df(pd.read_csv(path))

@st.cache_data(show_spinner=False, ttl=600)
def load_mistakes_source(name: str) -> pd.DataFrame | None:
    """
    Load a catalog mistakes source by logical name.
    Accepts: folder named <n>/  OR  zip named <n>.zip
    Silently returns None if neither exists (mistakes are optional).
    Supports xlsx, xls, and csv files inside the folder/zip.
    If multiple files found, they are concatenated and re-aggregated by Attribute.
    """
    d = Path(name)
    z = Path(f"{name}.zip")
    ...
    if d.exists() and d.is_dir():
        base = d
    elif z.exists() and z.is_file():
        extract_root = Path(".kpi_cache")
        extract_root.mkdir(exist_ok=True)
        base = ensure_zip_extracted(z, extract_root)
    else:
        return None  # silently skip — mistakes files are optional

    files = sorted(
        list(base.rglob("*.xlsx")) + list(base.rglob("*.xls")) + list(base.rglob("*.csv"))
    )
    if not files:
        return None
    ...
    # If multiple files were combined, re-aggregate by Attribute
    if len(dfs) > 1 and "Attribute" in combined.columns and "Total Failures" in combined.columns:
        combined = combined.groupby("Attribute", as_index=False)["Total Failures"].sum()
        total = combined["Total Failures"].sum()
        if total > 0:
            combined["Failure %"] = (combined["Total Failures"] / total * 100).round(2)
...
st.caption(
    "For each region/type, create a **folder** named exactly as listed (or a **.zip** with the same name) "
    "and place your xlsx/csv file(s) inside. Available sources: "
    + ", ".join(f"`{n}`" for n in MISTAKES_SOURCES)
)

# Load all available sources — silently skip missing ones
mistakes_data: dict[str, pd.DataFrame] = {}
for src_name in MISTAKES_SOURCES:
    df_m = load_mistakes_source(src_name)
    if df_m is not None and not df_m.empty:
        label = MISTAKES_LABELS.get(src_name, src_name)
        mistakes_data[label] = df_m

if not mistakes_data:
    st.info(
        "No catalog mistakes sources found yet. "
        "Create a folder (or zip) for each source you have — e.g. `Catalog Mistakes UAE All/` — "
        "and place your xlsx or csv file inside."
    )
else:
