...
# ✅ Your KPI “names” (we will look for either: a folder with this name OR a zip named "<name>.zip")
...
def extract_month_from_filename(stem: str) -> str:
    """
    Returns 'YYYY-MM' if found; else returns stem.
    Accepts: 2026-01, 2026_01, 202601, 'Tickets_2026-01', etc.
    """
    s = stem.strip()
...
def resolve_source(name: str) -> tuple[str, Path]:
    """
    Accepts your logical name (e.g. 'Tickets by City').
    Returns (kind, path):
      - ('dir', Path(name)) if folder exists
      - ('zip', Path(name + '.zip')) if zip exists
    """
    d = Path(name)
    z = Path(f"{name}.zip")
    ...
    if z.exists() and z.is_file():
        return ("zip", z)
    # also allow user to pass direct zip name as "Tickets by City.zip"
    z2 = Path(name)
...
def ensure_zip_extracted(zip_path: Path, extract_root: Path) -> Path:
    """
    Extract zip into extract_root/<zip_stem>/ (cached).
    Returns extracted folder path.
    """
    out_dir = extract_root / zip_path.stem
    out_dir.mkdir(parents=True, exist_ok=True)

    # A cheap "already extracted" check: if any CSV exists, skip
...
def read_kpi_source(name: str) -> pd.DataFrame:
    """
    Reads KPI data from either:
      - folder: <name>/...csv
      - zip:    <name>.zip (extracts to .kpi_cache/)
    Adds:
      - __month__ from csv filename
      - __source_file__ original csv name
    """
...
def metrics_from_report(df: pd.DataFrame, mode: str, level: str) -> pd.DataFrame | None:
    """
    Generic KPI table extractor:
    Returns columns: level | tickets | avg_score (if exists)
    level: 'City' or 'Market' or 'Name'
    """
...
def combine_kpis_by_level(raw_df: pd.DataFrame, mode: str, level: str) -> pd.DataFrame:
    """
    Combine monthly KPI exports:
      - tickets: sum
      - avg_score: weighted avg by tickets
    Returns: level | tickets | avg_score (if avg_score exists in source)
    """
...
def load_data(mode: str):
    ...
    # ✅ KPI raw monthly from ZIP sources
...
# Month filter from Evaluation Report
...
# Apply month filter to evaluation rows
if sel_months:
...
# Apply month filter to KPI raw sources (based on CSV filenames inside ZIP)
...
# Combine KPI exports for selected months
...
with c2:
    ...
    if src is None or src.empty or "avg_score" not in src.columns:
        st.info("No city score data found in KPI exports (or Avg QC Score column not detected).")
    else:
        cm = src.copy()
        if sel_city:
...
with c3:
    ...
    if src is None or src.empty or "tickets" not in src.columns:
        st.info("No city ticket data found in KPI exports (or Tickets column not detected).")
    else:
        cm = src.copy()
        if sel_city:
            cm = cm[cm["City"].isin(sel_city)]
...
with m1:
    ...
    if src is None or src.empty or "avg_score" not in src.columns:
        st.info("No market score data found in KPI exports (or Avg QC Score column not detected).")
    else:
        mm = src.copy()
        if sel_market:
...
with m2:
    ...
    if src is None or src.empty or "tickets" not in src.columns:
        st.info("No market ticket data found in KPI exports (or Tickets column not detected).")
    else:
        mm = src.copy()
        if sel_market:
            mm = mm[mm["Market"].isin(sel_market)]
...
def render_agent_scores(df: pd.DataFrame, title_hint: str):
    ...
    if "Name" not in a.columns:
        st.warning(f"{title_hint}: Name column not found after combining. Showing raw preview.")
